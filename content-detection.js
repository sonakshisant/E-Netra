...
class ContentDetectionModule {
  ...
  _analyzeChangeGroup(mutationGroup) {
    ...
    let target = mutationGroup[0].target;
    
    // Handle text nodes by promoting to parent element
  ...
  _shouldIgnoreElement(element) {
    // Handle text nodes by recursing to parent
    if (element && element.nodeType === Node.TEXT_NODE && element.parentElement) {
      return this._shouldIgnoreElement(element.parentElement);
    }
  ...
  _extractContentFromChange(mutationGroup, changeType) {
    ...
    switch (changeType) {
      ...
      case 'removal':
        // Extract content from removed nodes
        for (const mutation of mutationGroup) {
          if (mutation.type === 'childList') {
    ...
    while (current && current !== document.body) {
      ...
      if (!section) {
        ...
        if (role === 'region' || role === 'article' || role === 'section') {
          section = this._getElementLabel(current) || current.textContent.substring(0, 50).trim();
        }
   ...
   * @private
   * @param {Element} element - DOM element
   * @returns {Object} - Position info (e.g., index among siblings)
   */
  ...
  _isFormElement(element) {
    const formTags = ['FORM', 'INPUT', 'SELECT', 'TEXTAREA', 'BUTTON', 'FIELDSET', 'LABEL', 'OUTPUT'];
  ...
  _isInteractiveElement(element) {
    ...
    if (interactiveTags.includes(element.tagName.toUpperCase())) return true;
   ...
   * @private
   * @param {Element} element - DOM element
   * @returns {boolean} - True if live region
   */
  ...
  /**
   * Notify registered listeners of significant changes
   * @private
   * @param {Object[]} significantChanges - Array of significant change objects
   */
  _notifyChangeListeners(significantChanges) {
    this.changeListeners.forEach(listener => {
      listener(significantChanges);
    });
  }
}