...
class ChangeSummarizer {
  ...
  _groupRelatedChanges(changes) {
    ...
    // First pass: group by exact same target (only if multiple)
    ...
    // Add target groups to result (only if length > 1)
    for (const group of targetGroups.values()) {
      if (group.length > 1) {
        groups.push(group);
        for (const ch of group) {
          assignedChanges.add(ch);
        }
      }
    }
    
    // Second pass: group by parent context (only if multiple)
    const contextGroups = new Map();
    
    for (const change of changes) {
      if (assignedChanges.has(change)) continue;
      
      const contextKey = change.context?.parentContext ? `${change.context.parentContext.heading}|${change.context.parentContext.section}` : '';
      if (contextKey) {
    ...
    // Add context groups to result (only if length > 1)
